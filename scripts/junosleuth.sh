...
ssh_raw() {
  ...
  for ((attempt=1; attempt<=SSH_RETRIES; attempt++)); do
    ...
    "${SSH[@]}" "$remote" < /dev/null >"$stdout_file" 2>"$stderr_file"
    rc=$?
...
remote_shell_cmd() {
  ...
  if [[ "$REMOTE_MODE" == "cli" ]]; then
    ...
    printf 'sh -c %s' "$(single_quote "$cmd")"
  fi
}
...
if [[ "$ACQUIRE_FILES" -eq 1 ]]; then
  ...
  acquire_one() {
    ...
    if ! scp -q -P "$PORT" -o "BatchMode=$BATCH_MODE" -o ConnectTimeout=10 -o StrictHostKeyChecking=ask \
      "${USER_NAME}@${HOST}:$remote" "$dest" < /dev/null >>"$OUT/meta/file_acquisition.log" 2>&1; then
      {
        echo "default_scp_failed=$remote"
        echo "retrying_legacy_scp=$remote"
      } >> "$OUT/meta/file_acquisition.log"
      scp -O -q -P "$PORT" -o "BatchMode=$BATCH_MODE" -o ConnectTimeout=10 -o StrictHostKeyChecking=ask \
        "${USER_NAME}@${HOST}:$remote" "$dest" < /dev/null >>"$OUT/meta/file_acquisition.log" 2>&1 || true
    fi
...
if [[ -n "$ACQUIRE_MEMORY" ]]; then
  ...
  stream_memory_region() {
    ...
    while (( remain > 0 )); do
      ...
      ssh "${SSH_OPTS[@]}" "$USER_NAME@$HOST" "$remote" < /dev/null > "$raw_chunk" 2> "$chunk_err" || rc=$?
  ...
  for pid in "${memory_pids[@]}"; do
    ...
    ssh_raw "$(remote_shell_cmd "ps -p $pid -o pid,ppid,uid,lstart,args 2>&1 || ps auxww 2>&1 | awk -v pid=$pid '\$2 == pid {print}'")" "$id_out" "$id_err" "$id_att" || true
    rm -f "$id_att"
