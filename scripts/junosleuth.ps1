...
function Get-RemoteShellCommand {
    ...
    "sh -c $(Get-ShellSingleQuoted $Command)"
}
...
if ($AcquireFiles) {
    ...
    function Acquire-RemoteFile([string]$RemotePath) {
        ...
        $scpLog = Join-Path $script:MetaDir "file_acquisition.log"
        & scp.exe -q -P $Port -o ("BatchMode={0}" -f $script:SshBatchMode) -o ConnectTimeout=10 -o StrictHostKeyChecking=ask `
            "$UserName@$HostName`:$RemotePath" $dest 2>> $scpLog
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path $dest)) {
            Add-Content $scpLog "default_scp_failed=$RemotePath"
            Add-Content $scpLog "retrying_legacy_scp=$RemotePath"
            & scp.exe -O -q -P $Port -o ("BatchMode={0}" -f $script:SshBatchMode) -o ConnectTimeout=10 -o StrictHostKeyChecking=ask `
                "$UserName@$HostName`:$RemotePath" $dest 2>> $scpLog
        }
